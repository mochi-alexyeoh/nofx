...
export const translations = {
  en: {
    ...
    // Footer
    footerTitle: 'Zess - AI Trading System',
    ...
    backtestPage: {
      ...
      starting: 'Starting...',
      actions: { pause: 'Pause', resume: 'Resume', stop: 'Stop' },
      runList: { title: 'Runs' },
      emptyStates: { noRuns: 'No runs yet', selectRun: 'Select a run to view details' },
      charts: { equityEmpty: 'No data yet' },
      decisionTrail: { emptyHint: 'The AI thought & execution log will appear once the run starts.' },
      detail: { deleteLabel: 'Delete', exportLabel: 'Export' },
      form: {
    ...
    faqSubtitle: 'Find answers to common questions about ZessAI',
    ...
    // ===== GETTING STARTED =====
    faqWhatIsNOFX: 'What is ZessAI?',
    faqWhatIsNOFXAnswer:
      'ZessAI is an AI-powered trading platform for cryptocurrency, US stock markets and commodities. It uses Advanced LLMs to analyze market data and make autonomous trading decisions. Key features include: multi-AI model support, multi-exchange trading, and visual strategy builder.',

    faqHowDoesItWork: 'How does ZessAI work?',
    faqHowDoesItWorkAnswer:
      'ZessAI works in 5 steps: 1) Configure AI models and exchange API credentials; 2) Create a trading strategy (coin selection, indicators, risk controls); 3) Create a "Trader" combining AI model + Exchange + Strategy; 4) Start the trader - it will analyze market data at regular intervals and make buy/sell/hold decisions; 5) Monitor performance on the dashboard. The AI uses Chain of Thought reasoning to explain each decision.',

    faqIsProfitable: 'Is ZessAI profitable?',
    faqIsProfitableAnswer:
      'AI trading is experimental and NOT guaranteed to be profitable. Cryptocurrency futures are highly volatile and risky. ZessAI is designed for educational and research purposes. We strongly recommend: starting with small amounts (10-50 USDT), never investing more than you can afford to lose, thoroughly testing before live trading, and understanding that past performance does not guarantee future results.',
    ...
    // ===== INSTALLATION =====
    faqHowToInstall: 'How do I install ZessAI?',
    ...
    faqUpdateNOFX: 'How do I update ZessAI?',
    faqUpdateNOFXAnswer:
    ...
    faqStopLossTakeProfit: 'Does ZessAI support stop-loss and take-profit?',
    ...
    faqMultipleTradersAnswer:
      'Yes! ZessAI supports running 20+ concurrent traders. Each trader can have different: AI model, exchange account, strategy, decision interval. Use this to A/B test strategies, compare AI models, or diversify across exchanges. Monitor all traders on the Competition page.',
    ...
    faqPortInUseAnswer:
      'Check what\'s using the port: "lsof -i :8080" (macOS/Linux) or "netstat -ano | findstr 8080" (Windows). Kill the process or change ports in .env: ZessAI_BACKEND_PORT=8081, ZessAI_FRONTEND_PORT=3001. Restart with "docker compose down && docker compose up -d".',
    ...
    faqBinancePositionModeAnswer:
      'Error: "Order\'s position side does not match user\'s setting". You\'re in One-way Mode but ZessAI requires Hedge Mode. Fix: 1) Close ALL positions first; 2) Binance Futures → Settings (gear icon) → Preferences → Position Mode → Switch to "Hedge Mode" (双向持仓); 3) Restart your trader.',

    faqBalanceShowsZero: 'Account balance shows 0',
    faqBalanceShowsZeroAnswer:
      'Funds are likely in Spot wallet, not Futures wallet. Solution: 1) In Binance, go to Wallet → Futures → Transfer; 2) Transfer USDT from Spot to Futures; 3) Refresh ZessAI dashboard. Also check: funds not locked in savings/staking products.',
    ...
    faqEncryptionDetails: 'What encryption does ZessAI use?',
    faqEncryptionDetailsAnswer:
      'ZessAI uses multiple encryption layers: 1) AES-256-GCM for database storage (API keys, secrets); 2) RSA-2048 for optional transport encryption (browser to server); 3) JWT for authentication tokens. Keys are generated during installation. Enable TRANSPORT_ENCRYPTION=true for HTTPS environments.',

    faqSecurityBestPractices: 'What are security best practices?',
    faqSecurityBestPracticesAnswer:
      'Recommended: 1) Use exchange API keys with IP whitelist and minimal permissions (Futures Trading only); 2) Use dedicated subaccount for ZessAI; 3) Enable TRANSPORT_ENCRYPTION for remote deployments; 4) Never share .env or data.db files; 5) Use HTTPS with valid certificates; 6) Regularly rotate API keys; 7) Monitor account activity.',

    faqCanNOFXStealFunds: 'Can ZessAI steal my funds?',
    faqCanNOFXStealFundsAnswer:
      'API keys are stored locally on YOUR machine, never sent to external servers. ZessAI only has the permissions you grant via API keys. For maximum safety: use API keys with trading-only permissions (no withdrawal), enable IP whitelist, use a dedicated subaccount.',
    ...
    faqCustomAIAPIAnswer:
      'Yes! ZessAI supports any OpenAI-compatible API. In Config → AI Models → Custom API: 1) Enter your API endpoint URL (e.g., https://your-api.com/v1); 2) Enter API key; 3) Specify model name. This works with self-hosted models, alternative providers, or Claude via third-party proxies.',

    faqAIHallucinations: 'What about AI hallucinations?',
    faqAIHallucinationsAnswer:
      'AI models can sometimes produce incorrect or fabricated information ("hallucinations"). ZessAI mitigates this by: 1) Providing structured prompts with real market data; 2) Enforcing JSON output format for decisions; 3) Validating orders before execution. However, AI trading is experimental - always monitor decisions and don\'t rely solely on AI judgment.',
    ...
    // ===== CONTRIBUTING =====
    faqHowToContribute: 'How can I contribute to ZessAI?',
    faqHowToContributeAnswer:
      'ZessAI is open-source and welcomes contributions! Ways to contribute: 1) Code - fix bugs, add features (check GitHub Issues); 2) Documentation - improve guides, translate; 3) Bug Reports - report issues with details; 4) Feature Ideas - suggest improvements. Start with issues labeled "good first issue". All contributors may receive airdrop rewards.',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        'Open ZessAI over HTTPS (or http://localhost during development) and avoid insecure iframes/reverse proxies so the browser can enable Web Crypto.',
    ...
    modelConfig: {
      ...
      allModelsUnified: 'All models unified via x402. Switch anytime after setup.',
      setupWallet: 'Setup Wallet',
      walletInfo: 'x402 uses USDC on Base chain. You need an EVM wallet.',
      ...
      claw402Connected: 'x402 Protocol Connected',
      claw402Unreachable: 'x402 Protocol Unreachable',
  ...
  zh: {
    ...
    backtestPage: {
      title: '回测实验室',
      subtitle: '选择模型与时间范围，重放完整 AI 决策流程。',
      start: '开始回测',
      starting: '启动中...',
      actions: { pause: '暂停', resume: '继续', stop: '停止' },
      runList: { title: '回测记录' },
      emptyStates: { noRuns: '暂无回测记录', selectRun: '请选择一条回测查看详情' },
      charts: { equityEmpty: '暂无数据' },
      decisionTrail: { emptyHint: '回测运行后会显示 AI 思考与执行记录。' },
      detail: { deleteLabel: '删除', exportLabel: '导出' },
      form: {
        aiModelLabel: 'AI 模型',
        selectAiModel: '选择 AI 模型',
        enabled: '已启用',
        disabled: '已禁用',
        symbolsLabel: '交易对（逗号分隔）',
        decisionTfLabel: '决策周期',
        cadenceLabel: '决策间隔（K线）',
        timeRangeLabel: '时间范围',
        initialBalanceLabel: '初始资金 (USDT)',
        feeLabel: '手续费 (bps)',
        slippageLabel: '滑点 (bps)',
        btcEthLeverageLabel: 'BTC/ETH 杠杆 (x)',
      ...
      toasts: {
        ...
        actionSuccess: '{action} {id} 操作成功。',
        actionFailed: '操作失败，请稍后再试。',
        confirmDelete: '确定删除回测 {id} 吗？此操作不可恢复。',
        deleteSuccess: '回测记录已删除。',
        deleteFailed: '删除失败，请稍后再试。',
        exportSuccess: '已导出 {id} 数据。',
        exportFailed: '导出失败。',
      },
    },
    ...
    // FAQ Page
    faqTitle: '常见问题',
    faqSubtitle: '查找关于 ZessAI 的常见问题解答',
    ...
    // ===== 入门指南 =====
    faqWhatIsNOFX: 'ZessAI 是什么？',
    faqWhatIsNOFXAnswer:
      'ZessAI 是一个开源的 AI 驱动交易操作系统，支持加密货币和美股市场。它使用大语言模型（LLM）如 DeepSeek、GPT、Claude、Gemini 来分析市场数据，进行自主交易决策。核心功能包括：多 AI 模型支持、多交易所交易、可视化策略构建器、回测系统。',

    faqHowDoesItWork: 'ZessAI 是如何工作的？',
    faqHowDoesItWorkAnswer:
      'ZessAI 分 5 步工作：1）配置 AI 模型和交易所 API 凭证；2）创建交易策略（币种选择、指标、风控）；3）创建"交易员"，组合 AI 模型 + 交易所 + 策略；4）启动交易员 - 它会定期分析市场数据并做出买入/卖出/持有决策；5）在仪表板上监控表现。AI 使用思维链（Chain of Thought）推理来解释每个决策。',

    faqIsProfitable: 'ZessAI 能盈利吗？',
    faqIsProfitableAnswer:
      'AI 交易是实验性的，不保证盈利。加密货币期货波动性大、风险高。ZessAI 仅用于教育和研究目的。我们强烈建议：从小额开始（10-50 USDT），不要投入超过承受能力的资金，在实盘交易前充分回测，并理解过去的表现不代表未来的结果。',
    ...
    faqSupportedAIModelsAnswer:
      'ZessAI 支持 7+ 种 AI 模型：DeepSeek（推荐性价比）、阿里云通义千问、OpenAI（GPT-5.2）、Anthropic Claude、Google Gemini、xAI Grok、Kimi（月之暗面）。您也可以使用任何 OpenAI 兼容的 API 端点。每个模型各有优势 - DeepSeek 性价比高，OpenAI 能力强但贵，Claude 擅长推理。',
    ...
    faqHyperliquidSetupAnswer:
      'Hyperliquid 是去中心化交易所，需要钱包认证。步骤：1）访问 app.hyperliquid.xyz；2）连接钱包；3）生成 API 钱包（推荐）或使用主钱包；4）复制钱包地址和私钥；5）在 ZessAI 中添加 Hyperliquid 交易所并填入凭证。无需 KYC，完全链上。',
    ...
    faqStopLossTakeProfit: 'ZessAI 支持止损止盈吗？',
    faqStopLossTakeProfitAnswer:
    ...
    faqMultipleTradersAnswer:
      '可以！ZessAI 支持运行 20+ 个并发交易员。每个交易员可以有不同的：AI 模型、交易所账户、策略、决策间隔。用于 A/B 测试策略、比较 AI 模型或跨交易所分散风险。在竞赛页面监控所有交易员。',
    ...
    faqPortInUseAnswer:
      '查看占用端口的进程：macOS/Linux 用 "lsof -i :8080"，Windows 用 "netstat -ano | findstr 8080"。终止进程或在 .env 中修改端口：ZessAI_BACKEND_PORT=8081、ZessAI_FRONTEND_PORT=3001。然后 "docker compose down && docker compose up -d" 重启。',
    ...
    faqBinancePositionModeAnswer:
      '错误："Order\'s position side does not match user\'s setting"。您处于单向持仓模式，但 ZessAI 需要双向持仓模式。修复：1）先平掉所有持仓；2）币安合约 → 设置（齿轮图标）→ 偏好设置 → 持仓模式 → 切换为"双向持仓"；3）重启交易员。',

    faqBalanceShowsZero: '账户余额显示 0',
    faqBalanceShowsZeroAnswer:
      '资金可能在现货钱包而非合约钱包。解决方案：1）在币安进入钱包 → 合约 → 划转；2）将 USDT 从现货划转到合约；3）刷新 ZessAI 仪表板。也检查：资金未被理财/质押产品锁定。',
    ...
    faqEncryptionDetails: 'ZessAI 使用什么加密？',
    faqEncryptionDetailsAnswer:
      'ZessAI 使用多层加密：1）AES-256-GCM 用于数据库存储（API 密钥、密钥）；2）RSA-2048 用于可选的传输加密（浏览器到服务器）；3）JWT 用于认证令牌。密钥在安装时生成。HTTPS 环境启用 TRANSPORT_ENCRYPTION=true。',

    faqSecurityBestPractices: '安全最佳实践是什么？',
    faqSecurityBestPracticesAnswer:
      '建议：1）使用带 IP 白名单和最小权限（仅合约交易）的交易所 API 密钥；2）为 ZessAI 使用专用子账户；3）远程部署启用 TRANSPORT_ENCRYPTION；4）切勿分享 .env 或 data.db 文件；5）使用有效证书的 HTTPS；6）定期轮换 API 密钥；7）监控账户活动。',

    faqCanNOFXStealFunds: 'ZessAI 会盗取我的资金吗？',
    faqCanNOFXStealFundsAnswer:
      'ZessAI 是开源的（AGPL-3.0 许可）- 您可以在 GitHub 审计所有代码。API 密钥存储在您的机器本地，从不发送到外部服务器。ZessAI 只有您通过 API 密钥授予的权限。为最大安全：使用仅交易权限（无提现）的 API 密钥，启用 IP 白名单，使用专用子账户。',
    ...
    faqCustomAIAPIAnswer:
      '可以！ZessAI 支持任何 OpenAI 兼容的 API。在配置 → AI 模型 → 自定义 API 中：1）输入 API 端点 URL（如 https://your-api.com/v1）；2）输入 API 密钥；3）指定模型名称。适用于自托管模型、替代提供商或通过第三方代理的 Claude。',

    faqAIHallucinations: 'AI 幻觉问题怎么办？',
    faqAIHallucinationsAnswer:
      'AI 模型有时会产生不正确或虚构的信息（"幻觉"）。ZessAI 通过以下方式缓解：1）提供带真实市场数据的结构化提示词；2）强制 JSON 输出格式；3）执行前验证订单。但 AI 交易是实验性的 - 始终监控决策，不要完全依赖 AI 判断。',
    ...
    // ===== 参与贡献 =====
    faqHowToContribute: '如何为 ZessAI 做贡献？',
    faqHowToContributeAnswer:
      'ZessAI 是开源项目，欢迎贡献！贡献方式：1）代码 - 修复 bug、添加功能（查看 GitHub Issues）；2）文档 - 改进指南、翻译；3）Bug 报告 - 详细报告问题；4）功能建议 - 提出改进意见。从标记为"good first issue"的问题开始。所有贡献者可能获得空投奖励。',
    ...
    environmentCheck: {
      ...
      unsupportedDesc:
        '请通过 HTTPS 或本机 localhost 访问 ZessAI，并避免嵌入不安全 iframe/反向代理，以符合浏览器的 Web Crypto 规则。',
    ...
    modelConfig: {
      ...
      selectAiModel: '① 选择 AI 模型',
      allModelsUnified: '所有模型通过 x402 统一调用，创建后可随时切换',
      setupWallet: '② 设置钱包',
      walletInfo: '💡 x402 使用 Base 链上的 USDC 付费，你需要一个 EVM 钱包',
      ...
      claw402Connected: 'x402 Protocol 已连接',
      claw402Unreachable: 'x402 Protocol 不可达',
  ...
  id: {
    ...
    // Footer
    footerTitle: 'Zess - Sistem Trading AI',
    ...
    backtestPage: {
      title: 'Backtest Lab',
      subtitle: 'Pilih model + rentang waktu untuk replay loop keputusan AI.',
      start: 'Mulai Backtest',
      starting: 'Memulai...',
      actions: { pause: 'Jeda', resume: 'Lanjut', stop: 'Stop' },
      runList: { title: 'Daftar Run' },
      emptyStates: { noRuns: 'Belum ada run', selectRun: 'Pilih run untuk lihat detail' },
      charts: { equityEmpty: 'Belum ada data' },
      decisionTrail: { emptyHint: 'Log keputusan AI akan muncul setelah run dimulai.' },
      detail: { deleteLabel: 'Hapus', exportLabel: 'Export' },
      form: {
        aiModelLabel: 'Model AI',
        selectAiModel: 'Pilih model AI',
        enabled: 'Aktif',
        disabled: 'Nonaktif',
        symbolsLabel: 'Simbol (dipisah koma)',
        decisionTfLabel: 'Decision TF',
        cadenceLabel: 'Interval keputusan (bar)',
      ...
      toasts: {
        ...
        invalidRange: 'Waktu akhir harus lebih besar dari waktu mulai.',
        startSuccess: 'Backtest {id} dimulai.',
        startFailed: 'Gagal memulai. Coba lagi nanti.',
        actionSuccess: '{action} {id} berhasil.',
        actionFailed: 'Operasi gagal. Coba lagi nanti.',
        confirmDelete: 'Hapus backtest {id}? Tindakan ini tidak bisa dibatalkan.',
        deleteSuccess: 'Run backtest dihapus.',
        deleteFailed: 'Gagal menghapus. Coba lagi nanti.',
        exportSuccess: 'Data {id} berhasil diekspor.',
    ...
    faqSubtitle: 'Temukan jawaban untuk pertanyaan umum tentang ZessAI',
    ...
    faqWhatIsNOFX: 'Apa itu ZessAI?',
    faqWhatIsNOFXAnswer: 'ZessAI adalah sistem operasi trading bertenaga AI open-source untuk pasar kripto dan saham AS. Ia menggunakan model bahasa besar (LLM) seperti DeepSeek, GPT, Claude, Gemini untuk menganalisis data pasar dan membuat keputusan trading secara otonom. Fitur utama: dukungan multi-model AI, trading multi-bursa, dan pembangun strategi visual.',
    faqHowDoesItWork: 'Bagaimana cara kerja ZessAI?',
    faqHowDoesItWorkAnswer: 'ZessAI bekerja dalam 5 langkah: 1) Konfigurasi model AI dan kredensial API bursa; 2) Buat strategi trading (pemilihan koin, indikator, kontrol risiko); 3) Buat "Trader" menggabungkan Model AI + Bursa + Strategi; 4) Mulai trader - dia akan menganalisis data pasar secara berkala dan membuat keputusan beli/jual/tahan; 5) Pantau performa di dasbor.',
    faqIsProfitable: 'Apakah ZessAI menguntungkan?',
    faqIsProfitableAnswer: 'Trading AI bersifat eksperimental dan TIDAK dijamin menguntungkan. Futures kripto sangat volatil dan berisiko. ZessAI dirancang untuk tujuan edukasi dan riset. Kami sangat menyarankan: mulai dengan jumlah kecil (10-50 USDT), jangan investasi melebihi yang sanggup Anda rugi, uji sebelum trading nyata.',
    ...
    faqSupportedAIModelsAnswer: 'ZessAI mendukung 7+ model AI: DeepSeek (direkomendasikan untuk biaya/performa), Qwen, OpenAI (GPT), Claude, Gemini, Grok, dan Kimi. Anda juga dapat menggunakan endpoint API yang kompatibel dengan OpenAI.',
    ...
    faqHowToInstall: 'Bagaimana cara menginstal ZessAI?',
    ...
    faqUpdateNOFX: 'Bagaimana cara memperbarui ZessAI?',
    ...
    faqHyperliquidSetupAnswer: 'Hyperliquid adalah bursa terdesentralisasi. Langkah: 1) Kunjungi app.hyperliquid.xyz; 2) Hubungkan wallet; 3) Buat API wallet; 4) Salin alamat dan private key; 5) Tambahkan di ZessAI. Tanpa KYC.',
    ...
    faqStopLossTakeProfit: 'Apakah ZessAI mendukung stop-loss dan take-profit?',
    ...
    faqMultipleTradersAnswer: 'Ya! ZessAI mendukung 20+ trader bersamaan. Gunakan untuk A/B test strategi, bandingkan model AI, atau diversifikasi lintas bursa.',
    ...
    faqPortInUseAnswer: 'Periksa proses yang menggunakan port. Ubah port di .env: ZessAI_BACKEND_PORT=8081, ZessAI_FRONTEND_PORT=3001.',
    ...
    faqBinancePositionModeAnswer: 'Anda dalam mode One-way tetapi ZessAI memerlukan Hedge Mode. Tutup semua posisi, beralih ke Hedge Mode, restart trader.',
    ...
    faqEncryptionDetails: 'Enkripsi apa yang digunakan ZessAI?',
    faqEncryptionDetailsAnswer: 'ZessAI menggunakan: 1) AES-256-GCM untuk penyimpanan database; 2) RSA-2048 untuk enkripsi transport opsional; 3) JWT untuk token autentikasi.',
    ...
    faqCanNOFXStealFunds: 'Bisakah ZessAI mencuri dana saya?',
    faqCanNOFXStealFundsAnswer: 'ZessAI open-source (lisensi AGPL-3.0) - Anda bisa audit semua kode. API key disimpan lokal di mesin ANDA, tidak pernah dikirim ke server eksternal.',
    ...
    faqCustomAIAPIAnswer: 'Ya! ZessAI mendukung API yang kompatibel dengan OpenAI. Masukkan URL endpoint, API key, dan nama model.',
    faqAIHallucinations: 'Bagaimana dengan halusinasi AI?',
    faqAIHallucinationsAnswer: 'ZessAI memitigasi dengan: prompt terstruktur, format output JSON, dan validasi order sebelum eksekusi. Namun trading AI tetap eksperimental.',
    ...
    faqHowToContribute: 'Bagaimana cara berkontribusi ke ZessAI?',
    faqHowToContributeAnswer: 'ZessAI open-source dan menyambut kontribusi! Cara: 1) Kode - perbaiki bug, tambah fitur; 2) Dokumentasi; 3) Laporan Bug; 4) Ide Fitur. Semua kontributor mungkin mendapat reward airdrop.',
    ...
    environmentCheck: {
      ...
      unsupportedDesc: 'Buka ZessAI melalui HTTPS (atau http://localhost saat pengembangan).',
    ...
    modelConfig: {
      ...
      selectAiModel: 'Pilih Model AI',
      allModelsUnified: 'Semua model terpadu via x402. Ganti kapan saja setelah setup.',
      setupWallet: 'Setup Wallet',
      walletInfo: 'x402 menggunakan USDC di Base chain. Anda memerlukan wallet EVM.',
      ...
      claw402Connected: 'x402 Protocol Terhubung',
      claw402Unreachable: 'x402 Protocol Tidak Dapat Dijangkau',
